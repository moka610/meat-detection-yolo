...
function addStep(slide, n, title, body, x, y, w, color) {
  rect(slide, x, y, w, 1.02, COLORS.offWhite, {
    line: { color: COLORS.hair, width: 1 },
  });
  rect(slide, x, y, 0.42, 1.02, color);
  text(slide, String(n), {
    x: x + 0.11,
    y: y + 0.37,
    w: 0.2,
...
function titleSlide() {
  ...
  chip(slide, "YOLO11s", 0.66, 4.28, 1.15, COLORS.wine);
  chip(slide, "Roboflow", 1.95, 4.28, 1.25, COLORS.teal);
  chip(slide, "Object Detection", 3.35, 4.28, 1.72, COLORS.blue);
...
function relatedWorkSlide() {
  ...
  cols.forEach((c, i) => {
    const x = 0.82 + i * 4.02;
    rect(slide, x, 2.24, 3.15, 2.1, COLORS.offWhite, {
      line: { color: COLORS.hair, width: 1 },
    });
    rect(slide, x, 2.24, 3.15, 0.12, c[2]);
    text(slide, c[0], {
      x: x + 0.24,
      y: 2.62,
      w: 2.5,
...
function challengesSlide() {
  ...
  ], 8.05, 1.55, 4.55, 3.25, { fontSize: 14 });
  sectionLabel(slide, "Key issue", 8.05, 5.23, 1.38, COLORS.teal);
...
function objectiveSlide() {
  ...
  goals.forEach((g, i) => {
    const x = 0.84 + i * 4.1;
    rect(slide, x, 4.02, 3.25, 1.6, COLORS.offWhite, {
      line: { color: COLORS.hair, width: 1 },
    });
    text(slide, g[0], {
      x: x + 0.24,
...
function methodFlowSlide() {
  ...
  steps.forEach((s, i) => {
    ...
    const y = row === 0 ? 1.75 : 4.18;
    addStep(slide, i + 1, s[0], s[1], x, y, 2.55, s[2]);
    if ((row === 0 && i < 3) || (row === 1 && i < 6)) {
      line(slide, x + 2.63, y + 0.51, 0.35, 0, COLORS.hair, 1.1, {
...
function datasetSlide() {
  ...
  addImageCrop(slide, ASSET.labels, 0.75, 1.55, 3.4, 3.4);
  text(slide, "対象クラス", {
    x: 4.55,
    y: 1.62,
    w: 2.0,
  ...
  classes.forEach((c, i) => chip(slide, c[0], 4.58 + (i % 2) * 1.7, 2.12 + Math.floor(i / 2) * 0.62, 1.35, c[1]));
  bulletBox(slide, [
    "画像中の肉だけをバウンディングボックスで指定",
    "画像全体ではなく、位置と種類を同時に学習",
    "train / validation / test に分割して評価",
    "Roboflowでデータセットのバージョンを管理",
  ], 4.62, 3.48, 3.65, 2.2, { fontSize: 11.5 });
  addImageCrop(slide, ASSET.valPred, 8.85, 1.55, 3.72, 3.72, [0.32, 0.2, 0.36, 0.46]);
  sectionLabel(slide, "Annotation", 8.85, 5.65, 1.55, COLORS.teal);
  text(slide, "BBoxがYOLOの教師信号になる", {
    x: 10.6,
    y: 5.69,
    w: 1.9,
    h: 0.15,
...
function roboflowSlide() {
  ...
  rect(slide, 7.45, 1.85, 4.7, 3.7, COLORS.offWhite, {
    line: { color: COLORS.hair, width: 1 },
  });
  text(slide, "オリジナリティ", {
    x: 7.82,
...
function yoloSlide() {
  ...
  rect(slide, 0.76, 1.52, 4.05, 4.6, COLORS.offWhite, {
    line: { color: COLORS.hair, width: 1 },
  });
  text(slide, "学習設定", {
    x: 1.05,
  ...
  text(slide, "入力", { x: 5.2, y: 2.02, w: 0.8, h: 0.16, fontSize: 10, color: COLORS.muted, bold: true });
  addImageCrop(slide, ASSET.lamb, 5.25, 2.36, 1.72, 1.72);
  line(slide, 7.08, 3.22, 0.52, 0, COLORS.teal, 1.4, { endArrowType: "triangle" });
  text(slide, "YOLO", { x: 7.75, y: 3.1, w: 0.72, h: 0.16, fontSize: 10, color: COLORS.wine, bold: true });
  line(slide, 8.45, 3.22, 0.55, 0, COLORS.teal, 1.4, { endArrowType: "triangle" });
  text(slide, "出力", { x: 9.08, y: 2.02, w: 0.8, h: 0.16, fontSize: 10, color: COLORS.muted, bold: true });
  rect(slide, 9.08, 2.42, 2.95, 1.47, COLORS.offWhite, { line: { color: COLORS.hair, width: 1 } });
  text(slide, "肉の位置\n種類ラベル\nconfidence", {
    x: 9.42,
    y: 2.69,
    w: 2.15,
...
function resultsSlide() {
  ...
  metrics.forEach((m, i) => {
    ...
    rect(slide, 2.28, y + 0.03, 2.3, 0.13, COLORS.hair);
    rect(slide, 2.28, y + 0.03, 2.3 * m.value, 0.13, m.color);
...
function examplesSlide() {
  ...
  labeledImage(slide, ASSET.lamb, "lamb 0.81", 0.84, 2.04, 2.55, 2.55, COLORS.teal);
  labeledImage(slide, ASSET.pork, "pork 0.58", 3.67, 2.04, 2.55, 2.55, COLORS.rose);
  labeledImage(slide, ASSET.lamb2, "lamb 0.55", 6.5, 2.04, 2.55, 2.55, COLORS.blue);
  labeledImage(slide, ASSET.hard, "例: 判断が難しい画像", 9.33, 2.04, 2.55, 2.55, COLORS.amber);
  const notes = [
  ...
  notes.forEach((n, i) => {
    const x = 1.02 + i * 5.92;
    rect(slide, x, 5.28, 4.95, 0.72, COLORS.offWhite, {
      line: { color: COLORS.hair, width: 1 },
    });
    text(slide, n[0], {
      x: x + 0.25,
